 ...
 **/
#[no_mangle]
pub unsafe fn gsasl_finish(_sctx: &mut SessionData) {
    todo!()
    /*
    if (*sctx).clientp != 0 {
        if !(*sctx).mech.is_null() && (*(*sctx).mech).client.finish.is_some()
           {
            (*(*sctx).mech).client.finish.expect("non-null function pointer")(sctx,
                                                                              (*sctx).mech_data);
        }
    } else if !(*sctx).mech.is_null() &&
                  (*(*sctx).mech).server.finish.is_some() {
        (*(*sctx).mech).server.finish.expect("non-null function pointer")(sctx,
                                                                          (*sctx).mech_data);
    }
    rpl_free((*sctx).anonymous_token as *mut libc::c_void);
    rpl_free((*sctx).authid as *mut libc::c_void);
    rpl_free((*sctx).authzid as *mut libc::c_void);
    rpl_free((*sctx).password as *mut libc::c_void);
    rpl_free((*sctx).passcode as *mut libc::c_void);
    rpl_free((*sctx).pin as *mut libc::c_void);
    rpl_free((*sctx).suggestedpin as *mut libc::c_void);
    rpl_free((*sctx).service as *mut libc::c_void);
    rpl_free((*sctx).hostname as *mut libc::c_void);
    rpl_free((*sctx).gssapi_display_name as *mut libc::c_void);
    rpl_free((*sctx).realm as *mut libc::c_void);
    rpl_free((*sctx).digest_md5_hashed_password as *mut libc::c_void);
    rpl_free((*sctx).qops as *mut libc::c_void);
    rpl_free((*sctx).qop as *mut libc::c_void);
    rpl_free((*sctx).scram_iter as *mut libc::c_void);
    rpl_free((*sctx).scram_salt as *mut libc::c_void);
    rpl_free((*sctx).scram_salted_password as *mut libc::c_void);
    rpl_free((*sctx).scram_serverkey as *mut libc::c_void);
    rpl_free((*sctx).scram_storedkey as *mut libc::c_void);
    rpl_free((*sctx).cb_tls_unique as *mut libc::c_void);
    rpl_free((*sctx).saml20_idp_identifier as *mut libc::c_void);
    rpl_free((*sctx).saml20_redirect_url as *mut libc::c_void);
    rpl_free((*sctx).openid20_redirect_url as *mut libc::c_void);
