 ...
 -> libc::c_int {
    ...
    state =
        calloc(1 as libc::c_int as libc::c_ulong,
               ::std::mem::size_of::<_Gsasl_login_server_state>() as
                   libc::c_ulong) as *mut _Gsasl_login_server_state;
...
pub unsafe extern "C" fn _gsasl_login_server_step(mut sctx:
 ...
 -> libc::c_int {
    ...
    match (*state).step {
        0 => {
            *output =
                strdup(b"User Name\x00" as *const u8 as *const libc::c_char);
            if (*output).is_null() {
                return GSASL_MALLOC_ERROR as libc::c_int
            }
            *output_len =
                strlen(b"User Name\x00" as *const u8 as *const libc::c_char);
            (*state).step += 1;
