...
pub unsafe extern "C" fn _gsasl_saml20_client_start(mut sctx:
 ...
 -> libc::c_int {
    ...
    state =
        calloc(::std::mem::size_of::<saml20_client_state>() as libc::c_ulong,
               1 as libc::c_int as libc::c_ulong) as *mut saml20_client_state;
    if state.is_null() { return GSASL_MALLOC_ERROR as libc::c_int }
    *mech_data = state as *mut libc::c_void;
    return GSASL_OK as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn _gsasl_saml20_client_step(mut sctx:
                                                       *mut Gsasl_session,
                                                   mut mech_data:
                                                       *mut libc::c_void,
                                                   mut input:
