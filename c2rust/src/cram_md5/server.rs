...
pub unsafe extern "C" fn _gsasl_cram_md5_server_step(mut sctx:
 ...
 -> libc::c_int {
    ...
    if input_len <= (16 as libc::c_int * 2 as libc::c_int) as libc::c_ulong {
        return GSASL_MECHANISM_PARSE_ERROR as libc::c_int
    }
    if *input.offset(input_len.wrapping_sub((16 as libc::c_int *
                                                 2 as libc::c_int) as
    ...
    res =
        gsasl_saslprep(password, 0 as Gsasl_saslprep_flags, &mut normkey,
                       0 as *mut libc::c_int);
    if res != GSASL_OK as libc::c_int { return res }
    cram_md5_digest(challenge, strlen(challenge), normkey, strlen(normkey),
                    hash.as_mut_ptr());
    rpl_free(normkey as *mut libc::c_void);
    if memcmp(&*input.offset(input_len.wrapping_sub((16 as libc::c_int *
                                                         2 as libc::c_int) as
                                                        libc::c_ulong) as
                                 isize) as *const libc::c_char as
                  *const libc::c_void,
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn _gsasl_cram_md5_server_finish(mut sctx:
                                                           *mut Gsasl_session,
                                                       mut mech_data:
                                                           *mut libc::c_void) {
    let mut challenge: *mut libc::c_char = mech_data as *mut libc::c_char;
